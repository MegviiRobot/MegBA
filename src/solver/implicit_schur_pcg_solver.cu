...
__global__ void implicitEMulx(const T *const *const valPtrs,
                      ...
                      const int *absolutePositionPoint, const int resDim,
  ...
  for (int i = 0; i < resDim; ++i) {
    ...
    for (int j = cameraDim; j < cameraDim + pointDim; ++j) {
      ...
      JpX += valI *
              xPtrs[absolutePositionPointLocal * pointDim + j - cameraDim];
    }
    // Jc.T * Jp * x
    for (int j = 0; j < cameraDim; ++j) {
      const T valI = valPtrs[i][errorNum * j + tid];
      Sum = valI * JpX;
      atomicAdd(&result[absolutePositionCameraLocal * cameraDim + j],
                Sum);
    }
  }
}
...
__global__ void implicitETMulx(const T *const *const valPtrs,
  ...
  for (int i = 0; i < resDim; ++i) {
    ...
    for (int j = 0; j < cameraDim; ++j) {
      const T valI = valPtrs[i][errorNum * j + tid];
      JcX += valI *
             xPtrs[absolutePositionCameraLocal * cameraDim + j];
    }
    // Jp.T * Jc * x
    for (int j = cameraDim; j < cameraDim + pointDim; ++j) {
      const T valI = valPtrs[i][errorNum * j + tid];
...
template <typename T>
__global__ void doubleWeightedPlusKernel(int nItem, T weightx, const T *x, T weighty, const T *y,
                                   T *z) {
  unsigned int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= nItem) return;
...
bool ImplicitSchurPCGSolverDistributedCUDA(
    ...
    const std::vector<const T **> &valPtrsDevice,
    const EdgeVector<T> &edges, const int resDim,
   const int cameraNum, const int pointNum, const int cameraDim,
   const int pointDim, const int hppRows, const int hllRows, const std::vector<T *> &hppCsrVal,
...
void implicitSchurMakeVDistributed(const std::vector<const T **> &valPtrsDevice,
                           const int pointNum, const int pointDim, const int resDim,
                           const int cameraDim, const int hppRows,
...
void implicitSchurSolveWDistributed(
    const std::vector<const T **> &valPtrsDevice,
    const EdgeVector<T> &edges, const int resDim,
   const int pointNum, const int pointDim, const int cameraDim,
...
bool ImplicitSchurPCGSolverDistributed(
   ...
   const std::vector<T *> &hppCsrVal, const std::vector<T *> &hllCsrVal,
    const std::vector<const T **> &valPtrsDevice,
   const std::vector<T *> &g, const EdgeVector<T> &edges, int resDim,
    int cameraDim, int cameraNum, int pointDim, int pointNum,
   int hppRows, int hllRows,
 ...
 bool PCG_success = ImplicitSchurPCGSolverDistributedCUDA(
     option, valPtrsDevice, edges, resDim, cameraNum, pointNum, cameraDim, pointDim,
     hppRows, hllRows, hppCsrVal, hllInvCsrVal, g, deltaX);

 // xp = {C-1} * gpoint - {C-1} * Jp.T * Jc * deltaXcamera
 implicitSchurSolveWDistributed(valPtrsDevice, edges, resDim, pointNum, pointDim, cameraDim, hppRows,
                                hllRows, hllInvCsrVal, g, deltaX);
...
void ImplicitSchurPCGSolver<T>::solve(
  ...
  for (int deviceRank = 0; deviceRank < worldSize; ++deviceRank) {
    hppCsrVal[deviceRank] = linearSystem.implicitEquationContainers[deviceRank].csrVal[0];
    hllCsrVal[deviceRank] = linearSystem.implicitEquationContainers[deviceRank].csrVal[1];
  ...
  ImplicitSchurPCGSolverDistributed(
      this->solverOption.solverOptionPCG, hppCsrVal, hllCsrVal,
      valPtrsDevice, g, edges, resDim,
      linearSystem.dim[0], linearSystem.num[0], linearSystem.dim[1],
      linearSystem.num[1], linearSystem.dim[0] * linearSystem.num[0],
