...
template <typename T>
__global__ void
JetVector_add_JetVector_Kernel(const unsigned int N, const unsigned int nItem,
...
template <typename T>
__global__ void
Jet_PVector_add_JetVector_Kernel(const unsigned int nItem, const T *f_res,
...
template <typename T>
__global__ void
Jet_PVector_add_Scalar_Vector_Kernel(const unsigned int nItem, const T *f_res,
                                     const int f_grad_position, const T *g_res,
                                     T *out_res, T *out_grad) {
  /*
                 * 1D block and grid
  ...
  for (int i = 0; i < MemoryPool::getWorldSize(); ++i) {
    ...
    if (f.getGradPosition() != -1) {
      ...
      Jet_PVector_add_Scalar_Vector_Kernel<T><<<gridAndDim[0], gridAndDim[1]>>>(
          nItem, f.getCUDAResPtr()[i], f.getGradPosition(), g.getCUDAResPtr()[i],
          out->getCUDAResPtr()[i], out->getCUDAGradPtr()[i]);
    } else {
      // f is not JPV
      cudaMemcpyAsync(out->getCUDAGradPtr()[i], f.getCUDAGradPtr()[i],
...
template <typename T>
__global__ void
Scalar_Vector_add_Scalar_Vector_Kernel(const unsigned int nItem, const T *f_res,
                                       const T *g_res, T *out_res) {
  /*
                 * 1D block and grid
   */
...
template <typename T>
__global__ void
JetVector_minus_Jet_PVector_Kernel(const unsigned int nItem, const T *f_res,
...
template <typename T>
__global__ void
JetVector_minus_Scalar_Vector_Kernel(const unsigned int nItem, const T *f_res,
                                     const T *g_res, T *out_res) {
  /*
                 * 1D block and grid
   */
...
template <typename T>
__global__ void
Scalar_Vector_minus_PJetVector_Kernel(const unsigned int nItem, const T *f_res,
  ...
  for (int i = 0; i < MemoryPool::getWorldSize(); ++i) {
    ...
    if (g.getGradPosition() != -1) {
      ...
      Scalar_Vector_minus_JetVector_Kernel<T><<<gridAndDim[0], gridAndDim[1]>>>(
          out->getGradShape(), nItem, f.getCUDAResPtr()[i], g.getCUDAResPtr()[i],
...
template <typename T>
__global__ void
JetVector_add_Scalar_Kernel(const unsigned int N, const unsigned int nItem,
...
template <typename T>
__global__ void
JetVector_minus_Scalar_Kernel(const unsigned int N, const unsigned int nItem,
                              const T *f_res, const T *f_grad, const T g,
                              T *out_res, T *out_grad) {
  /*
                 * 1D block and grid
...
template <typename T>
__global__ void
Scalar_minus_JetVector_Kernel(const unsigned int N, const unsigned int nItem,
...
template <typename T>
__global__ void
Scalar_divides_JetVector_Kernel(const unsigned int N, const unsigned int nItem,
                                const T f, const T *g_res, const T *g_grad,
                                T *out_res, T *out_grad) {
  /*
                 * 1D block and grid
