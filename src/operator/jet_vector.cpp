...
template <typename T>
JetVector<T> &JetVector<T>::to(Device device) {
  switch (device) {
    case Device::CPU:
      CPU();
      break;
...
template <typename T>
bool JetVector<T>::IsEmpty() {
  return _valueHostVec.empty() && _valueDevicePtr.empty();
}

template <typename T>
void JetVector<T>::setGradShape(unsigned int N) {
  if (_N != 0)
    throw std::runtime_error(
        "Can not set Grad Shape on a working JetVector, "
...
template <typename T>
void JetVector<T>::setGradPosition(int gradPosition) {
...
template <typename T>
void JetVector<T>::appendJet(T a, int n) {
  /*
   * This method not support negative index so far. If n < 0, it has the same
   * effect as a _pureScalar. 'if (std::abs(n) > _N)' guarantees a negative
   * index won't raise error.
   */
  assert(_N != 0 &&
         "JetVector does not allow insert Jet "
  ...
  for (int i = 0; i < _N; ++i) _gradHostVec[i].push_back(i == n ? 1 : 0);
  _nItem++;
}

template <typename T>
void JetVector<T>::appendJet(T a) {
...
template <typename T>
JetVector<T> JetVector<T>::operator-() const {
  return T(0) - *this;
}
  ...
  switch (z.getDevice()) {
    case MegBA::Device::CPU: {
      ...
      for (unsigned int i = 0; i < z.getGradShape(); ++i) {
        s << "Grad[" << i << "]: "
          << "[ ";
        for (auto &data : z.getCPUGrad()[i]) s << data << ", ";
        s << "]," << std::endl;
      }
