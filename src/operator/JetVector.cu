...
namespace MegBA {
    ...
    void JetVector<T>::CUDA2CPU(const JetVector<T> &f) {
        ...
        for (int i = 0; i < world_size; ++i) {
            ...
            cudaMemcpyAsync(&_haData[start_idx], f._daPtr[i], nElm * sizeof(T), cudaMemcpyDeviceToHost);
            if (_gradPosition == -1) {
                for (unsigned int j = 0; j < _N; ++j)
                    cudaMemcpyAsync(&_hvData[j][start_idx], &f._dvPtr[i][j * nElm], nElm * sizeof(T), cudaMemcpyDeviceToHost);
            }
            start_idx += nElm;
        }
    }
    ...
    void JetVector<T>::CPU2CUDA(const JetVector<T> &f) {
        ...
        if (_daPtr.empty()) {
            if (_pureScalarFlag) {
              _daPtr.resize(world_size);
                ...
                for (int i = 0; i < world_size; ++i) {
                    ...
                    std::size_t nElm{getElmNum(i)};
                    cudaMemcpyAsync(_daPtr[i], &f._haData[start_idx], nElm * sizeof(T), cudaMemcpyHostToDevice);
                    start_idx += nElm;
                }
                return;
            }
            std::vector<void *> da_ptr{}, dv_ptr{};
            MemoryPool::allocateJetVector(da_ptr, dv_ptr, _N, _nElm,
                                           sizeof(T));
            // _dvPtr must be nullptr
            _dvPtr.clear();
            _dvPtr.reserve(world_size);
            for (int i = 0; i < world_size; ++i)
              _dvPtr.push_back((T *)dv_ptr[i]);

            _daPtr.clear();
            _daPtr.reserve(world_size);
            for (int i = 0; i < world_size; ++i)
              _daPtr.push_back((T *)da_ptr[i]);
            ...
            for (int i = 0; i < world_size; ++i) {
                ...
                cudaMemcpyAsync(_daPtr[i], &f._haData[start_idx], nElm * sizeof(T), cudaMemcpyHostToDevice);
                for (unsigned int j = 0; j < _N; ++j)
                    cudaMemcpyAsync(&_dvPtr[i][j * nElm], &f._hvData[j][start_idx], nElm * sizeof(T), cudaMemcpyHostToDevice);
                start_idx += nElm;
            }
        } else {
            std::size_t start_idx{0};
    ...
    void JetVector<T>::CUDA2CUDA(const JetVector<T> &f) {
        ...
        for (int i = 0; i < world_size; ++i) {
            cudaSetDevice(i);
            std::size_t nElm{getElmNum(i)};
            cudaMemcpyAsync(_daPtr[i], f._daPtr[i], nElm * sizeof(T), cudaMemcpyDeviceToDevice);
            cudaMemcpyAsync(_dvPtr[i], f._dvPtr[i], _N * nElm * sizeof(T), cudaMemcpyDeviceToDevice);
        }
    }
    ...
    std::ostream &ostreamCUDA(std::ostream &s, const JetVector<T> &z) {
        ...
        for (int i = 0; i < MemoryPool::getWorldSize(); ++i) {
            ...
            cudaMemcpyAsync(&Res[start_idx], z.getCUDAResPtr()[i], nElm * sizeof(T), cudaMemcpyDeviceToHost);
            for (unsigned int j = 0; j < N; ++j)
                cudaMemcpyAsync(&Grad[j][start_idx], &z.getCUDAGradPtr()[i][j * nElm], nElm * sizeof(T), cudaMemcpyDeviceToHost);
            start_idx += nElm;
        }
        s << "[Res: " << "[ ";
    ...
    template std::ostream &ostreamCUDA<float>(std::ostream &s, const JetVector<float> &);
    template std::ostream &ostreamCUDA<double>(std::ostream &s, const JetVector<double> &);
