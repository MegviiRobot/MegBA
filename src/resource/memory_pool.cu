...
void MemoryPool::resetPool(int N, std::size_t nEle, std::int8_t sizeofType,
                           int worldSize) {
...
void MemoryPool::allocateJetVector(std::vector<void *> *daPtr,
                                   ...
                                   std::size_t nEle, std::int8_t sizeofType) {
  std::unique_lock<std::mutex> lock{_mutex};
  daPtr->clear();
  daPtr->resize(_worldSize);
  dvPtr->clear();
  dvPtr->resize(_worldSize);
  assert((N == _N || N == 0) && nEle == _nEle && sizeofType == _sizeofType);
  for (auto offset : memOffsetCounter)
    if (offset != 0)
      throw std::runtime_error("memory leak");
  if (_ptr.empty()) {
    for (int i = 0; i < _worldSize; ++i) {
      const auto nEle = getEleNum(i);
      cudaSetDevice(i);
      Ptr ptr{nullptr};
      cudaMalloc(&ptr.address, (_N + 1) * nEle * _sizeofType);
      dvPtr->operator[](i) = ptr.address;
      ptr.number += _N * nEle * _sizeofType;
...
void MemoryPool::redistribute() {
  if (_poolSize.empty()) {
    ...
    for (int i = 0; i < _worldSize; ++i) {
      ...
      _poolSize[i] = (_N + 1) * nEle * _sizeofType * _ptr.size();
      cudaMalloc(&_headPtr[i], _poolSize[i]);
      int64_t offset{0};
      for (auto &item : _ptr) {
        Ptr ptr{_headPtr[i]};
        ptr.number += offset;
        offset += (_N + 1) * nEle * _sizeofType;
        item[i] = ptr.address;
      }
    }
  } else {
