...
BaseProblem<T>::BaseProblem(const ProblemOption& option) : option(option), solver(dispatchSolver(*this)) {
  if (option.N != -1 && option.nEle != -1)
    MemoryPool::resetPool(option.N, option.nEle, sizeof(T), option.deviceUsed.size());
  if (option.useSchur) {
    schurWS.splitSize = option.nEle / option.deviceUsed.size() + 1;
