...
namespace MegAutoBA {
    void CholmodSolverImpl(const double *csrVal, const int *csrColInd, const int *csrRowPtr,
                           const double *b,
                           std::size_t rows, std::size_t cols, std::size_t nnz, bool ReAnalyze,
                           double *delta_x) {
        ...
        static std::unique_ptr<cholmod_factor, std::function<void(cholmod_factor *&)>> Factor{
                cholmod_analyze(&Sparse, Common.get()), [Common = Common](cholmod_factor *&factor) mutable {
        ...
        static std::shared_ptr<cholmod_dense> Y_guard{Y, [Common = Common, Y_changed = Y_changed](cholmod_dense *&y) mutable {
                                                          if (!(*Y_changed))
                                                              cholmod_free_dense(&y, Common.get());
                                                          Y_changed.reset();
                                                          Common.reset();
                                                      }
        };
        ...
        cholmod_solve2(CHOLMOD_A, Factor.get(), &B, nullptr, &X, nullptr, &Y, nullptr, Common.get());
        if (Y != Y_guard.get()) {
            *Y_changed = true;
            Y_guard.reset(Y, [Common = Common, Y_changed = Y_changed](cholmod_dense *&y) mutable {
                if (!(*Y_changed))
                    cholmod_free_dense(&y, Common.get());
                Y_changed.reset();
                Common.reset();
