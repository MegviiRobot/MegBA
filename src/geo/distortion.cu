...
__global__ void RadialDistortionNoGradKernel(
    const int nEle, const int N, const T *px_da_ptr, const T *py_da_ptr,
  ...
  for (unsigned int i = 0; i < N; ++i)
    dv_ptr[tid + nEle * i] = partial * (px_dv_ptr[tid + nEle * i] * px +
                                        py_dv_ptr[tid + nEle * i] * py);
...
template <typename T>
__global__ void
RadialDistortionKernel(const int nEle, const int N, const T *px_da_ptr,
  ...
  T partial = 2 * f * (k1 + 2 * k2 * l2_pow2);
  for (unsigned int i = 0; i < N; ++i) {
    unsigned int index = tid + nEle * i;
    dv_ptr[index] =
        partial *
            (px_dv_ptr[tid + nEle * i] * px + py_dv_ptr[tid + nEle * i] * py) +
...
__global__ void RadialDistortionFastGradKernel(
  ...
  for (unsigned int i = 0; i < N; ++i) {
    unsigned int index = tid + nEle * i;
    dv_ptr[index] = partial * (px_dv_ptr[tid + nEle * i] * px +
                               py_dv_ptr[tid + nEle * i] * py) +
