    ...
    namespace geo {
        namespace {
            ...
            struct R2Q_Address_Wrapper<float> {
                ...
                static __device__ __host__ const float *const (&get_dR())[3][3] { return f_dR; }
            ...
            struct R2Q_Address_Wrapper<double> {
                static __device__ __host__ const double *const (&get_R())[3][3] { return d_R; }
                static __device__ __host__ const double *const (&get_dR())[3][3] { return d_dR; }
            ...
            __global__ void RotationToQuaternion(const int nItem, const int N) {
                ...
                if(inv_qw > 0) {
                    ...
                    for (int i = 0; i < N; ++i) {
                        ...
                        // x
                        W::get_dQ()[0][index] = ((W::get_dR()[1][2][index] - W::get_dR()[2][1][index]) * qw_mul_4 - 4 * dqw * (r[1][2] - r[2][1])) * inv_4q0_pow2;
                        // y
                        W::get_dQ()[1][index] = ((W::get_dR()[2][0][index] - W::get_dR()[0][2][index]) * qw_mul_4 - 4 * dqw * (r[2][0] - r[0][2])) * inv_4q0_pow2;
                        // z
                        W::get_dQ()[2][index] = ((W::get_dR()[0][1][index] - W::get_dR()[1][0][index]) * qw_mul_4 - 4 * dqw * (r[0][1] - r[1][0])) * inv_4q0_pow2;
                    }
                } else {
                    ...
                    for (int n = 0; n < N; ++n) {
                        ...
                        // w
                        W::get_dQ()[3][index] = ((W::get_dR()[k][j][index] - W::get_dR()[j][k][index]) * qw_mul_4 - 4 * dq0 * (r[k][j] - r[j][k])) * inv_4q0_pow2;
                        W::get_dQ()[j][index] = ((W::get_dR()[j][i][index] + W::get_dR()[i][j][index]) * qw_mul_4 - 4 * dq0 * (r[j][i] + r[i][j])) * inv_4q0_pow2;
                        W::get_dQ()[k][index] = ((W::get_dR()[k][i][index] + W::get_dR()[i][k][index]) * qw_mul_4 - 4 * dq0 * (r[k][i] + r[i][k])) * inv_4q0_pow2;
                    }
                }
            }

            template<typename T>
            __global__ void Normalize_(const int nItem, const int N,
                                       T *Qx, T *Qy, T *Qz, T *Qw,
                ...
                const T inv_l2 = Wrapper::rsqrtG<T>::call(qw * qw + qx * qx + qy * qy + qz * qz) * sign;
                ...
                for (int i = 0; i < N; ++i) {
                    ...
                    const T dqy = dQy[index];
                    const T dqz = dQz[index];
                    const T common_coeff = inv_l2 * inv_l2 * (qw * dqw + qx * dqx + qy * dqy + qz * dqz);
