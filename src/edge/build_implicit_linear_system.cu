...
__global__ void makeHppHllSchur(
    ...
    const int resDim, const int cameraDim, const int pointDim,
    const int errorNum, T *gCamera, T *gPoint, T *hppCsrVal, T *hllCsrVal) {
  /*
...
void EdgeVector<T>::buildImplicitLinearSystemCUDA(
  ...
  for (int i = 0; i < option.deviceUsed.size(); ++i) {
    ...
    ASSERT_CUDA_NO_ERROR();
    cudaMemsetAsync(
        linearSystemLocal.implicitEquationContainers[i].csrVal[0], 0,
        linearSystemLocal.implicitEquationContainers[i].nnz[0] * sizeof(T));
    cudaMemsetAsync(
        linearSystemLocal.implicitEquationContainers[i].csrVal[1], 0,
  ...
  if (jetInformation.rows() != 0 && jetInformation.cols() != 0) {
    ...
    for (int i = 0; i < option.deviceUsed.size(); ++i) {
      ...
      makeHppHllSchur<<<grid, block, block.x * block.y * sizeof(T)>>>(
          ...
          positionContainers[i].absolutePosition[1], resDim, cameraDim,
          pointDim, edgeNum, gCameraDevice[i], gPointDevice[i],
