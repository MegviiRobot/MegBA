...
namespace MegBA {
    ...
    public:
      ...
      JetVector(const JetVector<T> &f) : _N(f._N), _nElm(f._nElm), _device(f._device), _haData(f._haData),
            _hvData(f._hvData) {
            switch (_device) {
                case CPU_t: {
                    break;
        ...
        JetVector(JetVector<T> &&f) noexcept : _N(std::move(f._N)), _nElm(std::move(f._nElm)),
              ...
              _gradPosition(std::move(f._gradPosition)),
        ...
        static const JetVector<T> &getInitTemplate(const JetVector<T> &f, const JetVector<T> &g) {
        ...
        std::size_t getElmNum(int rank) const {
        ...
        void bindDaPtr(std::vector<T *> &&da_ptr) {
          _daPtr = std::move(da_ptr); };
