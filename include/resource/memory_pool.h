...
class MemoryPool {
  ...
  static int _N;
  static std::size_t _nEle;
  static std::uint8_t _sizeofType;
  static int _worldSize;
  static std::size_t _ptrInUseCounter;

 public:
  static void resetPool(int N, std::size_t nEle, std::int8_t sizeofType,
                        int worldSize);
  ...
  static std::size_t getEleNum(int rank) {
    if (rank == _worldSize - 1)
      return _nEle - (_nEle / _worldSize + 1) * (_worldSize - 1);
    else
      return _nEle / _worldSize + 1;
  }

  static std::size_t getEleNum(int rank, std::size_t nEle) {
    if (rank == _worldSize - 1)
      return nEle - (nEle / _worldSize + 1) * (_worldSize - 1);
    else
      return nEle / _worldSize + 1;
  }
};
}  // namespace MegBA
