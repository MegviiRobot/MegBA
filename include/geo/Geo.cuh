...
namespace MegBA {
    namespace geo {
        ...
        template<typename T>
        JM33<T> AngleAxisToRotationKernelMatrix(const Eigen::Map<const JVD<T>> &AxisAngle);
        ...
        template<typename T>
        JetVector<T> RadialDistortion(const JV3<T> &point, const Eigen::Map<const JV3<T>> &intrinsic);

        template<typename T>
        JetVector<T> RadialDistortion(const JV3<T> &point, const Eigen::Map<const JVD<T>> &intrinsic);
    }
}
