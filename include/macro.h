...
#if DEBUG
#define PRINT_DMEMORY(d_ptr, nEle, T)                                     \
do {                                                                      \
    T *__ptr = new T[nEle];                                               \
    cudaMemcpy(__ptr, d_ptr, (nEle) * sizeof(T), cudaMemcpyDeviceToHost); \
    std::cout << #d_ptr << ": ";                                          \
    for (std::size_t __i = 0; __i < (nEle); ++__i)                        \
        std::cout << __ptr[__i] << " ";                                   \
    std::cout << std::endl;                                               \
    delete[] __ptr;                                                       \
} while(false)
...
#define PRINT_DMEMORY_SEGMENT(d_ptr, start, nEle, T)                                 \
do {                                                                                \
    assert((nEle) > 0);\
    cudaDeviceSynchronize();                                                        \
    T *__ptr = new T[nEle];                                                        \
    cudaMemcpy(__ptr, &(d_ptr)[start], nEle * sizeof(T), cudaMemcpyDeviceToHost);         \
    std::cout << (nEle) << " elements from " << (start) <<"-th in " << #d_ptr << ": " << std::endl;   \
    for (std::size_t __i = 0; __i < (nEle); ++__i)                            \
        std::cout << __ptr[__i] << " ";                                     \
    std::cout << std::endl;                                             \
    delete[] __ptr;\
...
do {                                                                \
    ...
    if (error != cudaSuccess)                                       \
        throw std::runtime_error(std::string("Failed at: ")         \
        + std::string(__FILE__)                                     \
...
do {                                                         \
    ...
    cudaMemcpy(h_csrRowPtr, csrRowPtr, ((rows_num) + 1) * sizeof(int), cudaMemcpyDeviceToHost);\
    ...
    Eigen::Map<Eigen::SparseMatrix<T>, Eigen::RowMajor> SpMat{(Eigen::Index)(rows_num), (Eigen::Index)(rows_num), (Eigen::Index)nnz, h_csrRowPtr, h_csrColInd, h_csrVal};\
    std::cout << #csrVal << ":\n";                                                      \
