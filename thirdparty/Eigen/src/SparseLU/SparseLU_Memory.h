...
/** 
  ...
  * \param vec Valid pointer to the vector to allocate_JetVector or expand
...
Index  SparseLUImpl<Scalar,StorageIndex>::expand(VectorType& vec, Index& length, Index nbElts, Index keep_prev, Index& num_expansions) 
{
  ...
  if(num_expansions == 0 || keep_prev) 
    new_len = length ; // First time allocate_JetVector requested
  else 
...
#endif
  {
    if (!num_expansions)
    {
      // First time to allocate_JetVector from LUMemInit()
...
Index SparseLUImpl<Scalar,StorageIndex>::memInit(Index m, Index n, Index annz, Index lwork, Index fillratio, Index panel_size,  GlobalLU_t& glu)
{
  ...
  // First allocate_JetVector Integer pointers for L\U factors
  glu.xsup.resize(n+1);
