...
public:
  using PureCoeffReturnType = std::decay_t<CoeffReturnType>;
  template<typename EvalType>
  using CastIfNotReferentType = std::conditional_t<std::is_reference<EvalType>::value,
          std::conditional_t<std::is_same<std::decay_t<EvalType>, PureCoeffReturnType>::value, EvalType, CoeffReturnType>,
          CoeffReturnType>;
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
#if EIGEN_HAS_CXX14
  auto coeff(Index row, Index col) const -> CastIfNotReferentType<decltype(m_argImpl.coeff(col, row))>
#else
  CoeffReturnType coeff(Index row, Index col) const
#endif
  {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
#if EIGEN_HAS_CXX14
  auto coeff(Index index) const -> CastIfNotReferentType<decltype(m_argImpl.coeff(index))>
#else
  CoeffReturnType coeff(Index index) const
#endif
...
{
  template <typename IndexType>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE auto operator()(const NullaryOp& op, IndexType=0, IndexType=0) const
  -> std::conditional_t<std::is_same<std::remove_reference_t<Scalar>&, decltype(op())>::value, std::remove_reference_t<Scalar>&, Scalar>
  { return op(); }
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
#if EIGEN_HAS_CXX14
  auto coeff(IndexType row, IndexType col) const -> CastIfNotReferentType<decltype(m_wrapper(m_functor, row, col))>
#else
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
#if EIGEN_HAS_CXX14
  auto coeff(IndexType index) const -> CastIfNotReferentType<decltype(m_wrapper(m_functor,index))>
#else
  ...
  : evaluator_base<CwiseBinaryOp<BinaryOp, Lhs, Rhs> >
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
  std::remove_const_t<CoeffReturnType> coeff(Index row, Index col) const
  {
    return m_d.func()(m_d.lhsImpl.coeff(row, col), m_d.rhsImpl.coeff(row, col));
  }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
  std::remove_const_t<CoeffReturnType> coeff(Index index) const
  {
...
struct mapbase_evaluator : evaluator_base<Derived>
{
  ...
  auto coeff(Index row, Index col) const
    -> std::conditional_t<std::is_same<std::decay_t<std::remove_pointer_t<PointerType>>, PureCoeffReturnType>::value,
            const PureCoeffReturnType &,
            CoeffReturnType>
#else
  CoeffReturnType coeff(Index row, Index col) const
#endif
