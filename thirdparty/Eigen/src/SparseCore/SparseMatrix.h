  ...
  : public SparseCompressedBase<SparseMatrix<Scalar_, Options_, StorageIndex_> >
{
  ...
  public:
    ...
    void conservativeResize(Index rows, Index cols) 
    {
      ...
      {
        // Inner size decreased: allocate_JetVector a new m_innerNonZeros
      ...
      // Re-allocate_JetVector outer index structure if necessary
      if (outerChange == 0)
        return;
...
typename SparseMatrix<Scalar_,Options_,StorageIndex_>::Scalar& SparseMatrix<Scalar_,Options_,StorageIndex_>::insert(Index row, Index col)
{
  ...
  if(isCompressed())
  {
    if(nonZeros()==0)
    {
      ...
      // and allocate_JetVector the entire free-space to the first inner-vector
  ...
  if(m_outerIndex[outer]==data_end)
  {
    ...
    // and allocate_JetVector the entire free-space to the current inner-vector.
...
EIGEN_DONT_INLINE typename SparseMatrix<Scalar_,Options_,StorageIndex_>::Scalar& SparseMatrix<Scalar_,Options_,StorageIndex_>::insertCompressed(Index row, Index col)
{
  ...
  if (m_data.allocatedSize()<=m_data.size())
  {
    ...
    else
    {
      ...
      //   2) avoid to allocate_JetVector too much memory when the matrix is almost empty
